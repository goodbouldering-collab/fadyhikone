...
3. **健康ログ入力**: 
   - 日付選択機能（過去の日付も入力可能）
   - 体重、体脂肪率、睡眠、運動時間の記録
   - **13種類の運動種目を個別記録**: ファディー、ストレッチ、筋トレ、ランニング、ジョギング、ウォーキング、サイクリング、水泳、ヨガ、ピラティス、HIIT、ダンス、ボクシング
   - **運動別のMET値に基づく消費カロリー自動計算**: 運動時間と体重から消費カロリーを自動計算
   - **朝食・昼食・夕食・間食**の分類別記録
   - **複数枚の食事写真**アップロード対応
   - **PFCバランス**（タンパク質・脂質・炭水化物）自動計算
   - 1日の合計カロリー・栄養素表示
   - **AI包括的アドバイス自動生成**: 保存時に全データ（食事+運動）を分析し、過去トレンド・今後の課題を含むアドバイスを自動作成
   - **総カロリー自動計算**: 朝食+昼食+夕食+間食の合計カロリーを自動計算・記録
   - **運動データの自動保存と表示**: 全運動種目のデータをexercise_activitiesテーブルに保存し、保存直後・ログ読み込み時も常に表示
   - **ログ上書き確認**: 既存ログがある日付に保存する際、確認ダイアログを表示
...
- meals: 食事詳細（朝昼夕間食別、カロリー、PFCバランス、AI解析データ）
- meal_photos: 食事写真（複数枚対応）
- exercise_activities: 運動アクティビティ詳細（運動種目、時間、強度、消費カロリー）
...
#### 包括的分析内容
1. **現在の状態分析**: 今日入力した全データ（体重、体脂肪、食事、PFC、運動詳細、睡眠）を総合評価
2. **運動データ分析**: 記録された各運動種目の時間・強度・消費カロリーを個別評価、摂取カロリーと消費カロリーのバランス分析
3. **過去トレンド分析**: 過去7日間の詳細データ + 過去30日間の統計データを比較
4. **今後の課題**: 短期目標（今週〜来週）と中期目標（今月〜来月）を提示、運動プログラムの最適化提案
5. **具体的アクション**: 数値を含む実行可能な推奨事項（カロリー調整、運動時間・種目の組み合わせ最適化など）
6. **クライミング的視点**: ファディー彦根の30年のクライミング経験を活かしたアドバイス
